...
function CharacterCustomisationPanel_Ingame:UpdatePlayerCustomisation()
	...
	local muscleVisualsHasChanged = self.data.muscleVisuals ~= self.muscleButton:isSelected()

	self.data.muscleVisuals = self.muscleButton:isSelected()

	if muscleVisualsHasChanged then FaceManager.SetPlayerMuscle(self.char) end
